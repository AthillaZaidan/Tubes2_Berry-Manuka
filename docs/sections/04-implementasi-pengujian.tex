...
\begin{enumerate}
    \item \textbf{Stage 1 (Builder)}: Menggunakan \textit{image} \texttt{golang:1.24-alpine}. Dependensi diunduh terlebih dahulu (\texttt{go mod download}) sebelum menyalin kode sumber. Binary dikompilasi dengan \texttt{CGO\_ENABLED=0} dan \texttt{GOOS=linux} untuk menghasilkan \textit{static binary}.
...
\begin{table}[H]
\centering
\caption{Test Case Parallel BFS dan Parallel DFS (Bagian 1)}
...
7 node \texttt{div} ditemukan, hasil identik BFS sekuensial &
7 node \texttt{div} ditemukan, identik &
\checkmark \\
\hline
\end{tabularx}
\end{table}
...
\begin{table}[H]
\centering
\caption{Test Case Parallel BFS dan Parallel DFS (Lanjutan)}
...
TC-PB-04 &
Selector: \texttt{*}, Limit: semua, Algoritma: Parallel DFS &
nilai depth setiap node identik dengan DFS sekuensial &
node sesuai &
\checkmark \\
\hline
\end{tabularx}
